...
void PublishData(HPublish pub, int n, RawData **fans)
{
	...
	if (skip > 0) {
		...
		RCLCPP_ERROR_THROTTLE(rclcpp::get_logger("bot_lidar"), 
			clock_, 5000, "[%s.%d]: drop %d fans.", 
...
int GetAllFans(HPublish pub, bool with_resample, double resample_res, RawData **fans)
{
	...
	{
		//printf("%d %d \n",hub->fans[i]->angle ,hub->fans[i]->span);
		if (hub->fans[i]->angle + hub->fans[i]->span == 1800)//3600
		{
			cnt = i + 1;
			break;
		}
					  ...
					  bool inverted, bool reversed)
{
	...
	if (with_filter)
	{
		...
		if (inverted)
		{
			...
			// std::cout << "with_filter: true, inverted: true, msg.angle_min: " << msg.angle_min << std::endl;
		}
		else
		{
			...
			// std::cout << "with_filter: true, inverted: false, msg.angle_min: " << msg.angle_min << std::endl;
		}
	}
	else
	{
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < lidar_count; i++)
	{
		...
		laser_pubs[i] = node->create_publisher<sensor_msgs::msg::LaserScan>(laser_topics[i], rclcpp::SystemDefaultsQoS());//rclcpp::SensorDataQoS()
	}
